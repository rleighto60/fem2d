...
description -   Routine to compute flux contribution to load vector.
...
description -   Finite element shape function routine based on actual shape 
...
void bcint(int m, REAL ke[18][18], REAL fe[18])
{
    float xi;
    REAL r,dxde,dyde,dval,J,fac,pen;
...
/* for axisymmetry... */
            ...
            if (ptype) { 
                for (i=0; i<nn; i++) 
                    r += (REAL)psi[i]*nodes[nnums[elems[ne].nodes[nds[i]]]].x;
                fac *= r;
            }
